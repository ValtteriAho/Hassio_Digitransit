...
- Bus stop IDs (for example `HSL:1010105`)
...
5. Add stop IDs and select the matching router for each stop.
...
The stop ID must match the selected router. For example, an HSL stop ID must use the `hsl` router.
